...
export function SendBar({ api, paths, resetPaths }: { api: SignatureRequestApi; paths: Record<string, string>; resetPaths: () => void }) {
  ...
  const openWhatsApp = (item: { name: string; phone: string | null; token: string; fileName: string }) => {
    ...
    window.open(
      intl ? `https://wa.me/${intl}?text=${encodeURIComponent(text)}` : `https://wa.me/?text=${encodeURIComponent(text)}`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  const onSend = async () => {
    ...
    try {
      ...
      const links = (created.recipients ?? [])
        ...
        .map((r) => ({ id: r.id, name: r.name, phone: r.phone, token: r.signing_token as string, fileName: created.fileName }));
      setShareLinks(links);
      toast.success("הבקשה נוצרה בהצלחה", { description: "ניתן לשלוח כעת בוואטסאפ או להעתיק קישור מהפעילות" });
