...
export function Step1Upload({ api, setPath, removePath }: { api: SignatureRequestApi; paths: Record<string, string>; setPath: (id: string, p: string) => void; removePath: (id: string) => void }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [drag, setDrag] = useState(false);
  const [busy, setBusy] = useState<string | null>(null);
  const validate = (f: File) => {
    const ext = "." + (f.name.split(".").pop()?.toLowerCase() ?? "");
    if (!ALLOWED.includes(ext)) { toast.error("סוג קובץ לא נתמך"); return false; }
    if (f.size > MAX) { toast.error("הקובץ גדול מ-20MB"); return false; }
    return true;
  };
  ...
  return (
    <StepCard step={1} title="העלאת מסמך" description="גרור קובץ או בחר מהמחשב (עד 20MB)">
      ...
      >
        {busy ? <Loader2 className="h-8 w-8 animate-spin text-primary" /> : <UploadCloud className="h-8 w-8 text-primary icon-glow" />}
        <p className="text-sm font-semibold text-foreground">{busy ? `מעלה ${busy}…` : "גרור לכאן או לחץ לבחירה"}</p>
      ...
      {api.files.length > 0 && (
        <ul className="mt-4 flex flex-col gap-2">
          {api.files.map((f) => (
            <li key={f.id} className="flex items-center gap-3 rounded-lg border border-primary/20 bg-primary/5 p-2.5">
              ...
              <button type="button" onClick={() => { api.removeFile(f.id); removePath(f.id); }}
                className="rounded-md p-1 text-muted-foreground hover:bg-destructive/15 hover:text-destructive" aria-label="הסר קובץ">
                <X className="h-4 w-4" />
              </button>
            </li>
          ))}
