...
-- Public peek (anon): minimal data for the verification screen
create or replace function public.peek_signing_token(_token text)
returns table(verification_type text, recipient_name text, document_subject text, already_signed boolean)
language sql
stable
security definer
set search_path = public
as $$
  select r.verification_type, r.name, d.subject, (r.status = 'signed')
...
-- Verify identity and return signing context
...
do $$ begin
  ...
  if not exists (select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='recipients') then
